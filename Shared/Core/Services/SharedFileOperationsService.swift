...
actor FanOutLiveProgressState {
    ...
    private var lastStage: ObservedStage?
    ...
    func observeChunk(
        ...
        let stageChanged = lastStage != .copying
    ...
    func observeVerification(
        ...
        let stageChanged = lastStage != .verifying
        guard stageChanged || now.timeIntervalSince(lastEmission) >= throttleInterval else {
            return nil
        }
        lastEmission = now
        lastStage = .verifying
...
class SharedFileOperationsService: FileOperationsService {
    ...
    private func executeOperation(
        ...
        let startTime = Date()
        // Perf 5: pipelined verification is an explicit service configuration.
        // The GUI initializer preserves its preference, while machine clients
        // such as the worker supply their own deterministic value.
        ...
        for entry in sourceManifest {
            ...
            do {
                copied = try await FileCopyService.copyFileFanOut(
                    ...
                    onVerificationChunk: { _, fileSize in
                        let now = Date()
                        guard let update = await liveProgressState.observeVerification(
                            fileSize: fileSize,
                            now: now,
                            throttleInterval: 0.5
                        ) else { return }
