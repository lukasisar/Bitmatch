...
New worker files are held under exclusive temporary names. The worker performs
ordinary synchronization, requests `F_FULLFSYNC`, and SHA-256-checks the
temporary bytes before an atomic `linkat` no-overwrite publication. It then
...
| Darwin `F_FULLFSYNC` | macOS accepted its stronger full-sync request. | Does not prove physical residence on NAND/platter across every bridge, filesystem, firmware, or device. |
...
`FAILED`. Strong requires every strong fact for every planned file. Unsupported
stronger syscalls can yield degraded evidence only after complete matching
readback; syscall failure, short read, mutation, collision, or checksum mismatch
fails. This classification is worker evidence, not Safe-to-clear policy.
