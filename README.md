...
Protocol V3 reports strong, degraded, or failed facts after temporary-file
SHA-256 verification, best-effort Darwin durability requests, no-overwrite
publication (including a fail-closed exFAT fallback), and a complete,
identity-bound, independently reopened destination readback. Any successful
outcome also requires exactly one result for every
...
See [the V3 protocol contract](docs/TRANSFER_WORKER_PROTOCOL.md) and
